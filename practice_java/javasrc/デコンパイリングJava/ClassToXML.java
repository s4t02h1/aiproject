...
class ClassParser {
	...
	void getVerificationType(RandomAccessFile ip, int number) throws IOException {
		int type, index, offset, verifyType;
		for(int i = 0; i < number; i++) {
			System.out.println("<VerificationTypeInfo>");
			verifyType = ip.readByte();
			if(verifyType < 7) {
				System.out.println("<VerificationType>" + verifyType + "</VerificationType>");
			} else if(verifyType == 7) {
				index = ip.readShort();
				System.out.println("<VerificationType>" + verifyType + "</VerificationType>");
				System.out.println("<CpoolIndex>" + index + "</CpoolIndex>");
			} else if(verifyType == 8) {
				offset = ip.readShort();
				System.out.println("<VerificationType>" + verifyType + "</VerificationType>");
	...
	void getAttributes(RandomAccessFile ip) {
		...
		int codeAttIndex, codeAttLength;
		String codeAttType;
		int frameCount, frameType;
		int numLocal, numStack;
...
//System.out.println("//FilePos:" + ip.getFilePointer() + "; Padding: " + padding + "\n");
				...
				// StackMapTable for Java6 is optional.

				while (attCount-- > 0)
				{

					codeAttIndex = ip.readShort();
					codeAttLength = ip.readInt();

					System.out.println("<CodeAttribute_Name_Index>" + codeAttIndex + "</CodeAttribute_Name_Index>");
					System.out.println("<CodeAttribute_Length>" + codeAttLength + "</CodeAttribute_Length>");
					codeAttType = new String(constantPool[codeAttIndex][1].substring(8));
					if(codeAttType.equals("LineNumberTable")) {
						...
						System.out.println("</LineNumTable>");
					} else if(codeAttType.equals("StackMapTable")) {
						frameCount = ip.readShort();
						System.out.println("<StackMapTable_Count>" + frameCount + "</StackMapTable_Count>");
						System.out.println("<StackMapTable>");
						while(frameCount-- > 0) {
							frameType = ip.readUnsignedByte();
							if(frameType < 64) {
								System.out.println("<SameFrame>");
								System.out.println("<OffsetDelta>" + frameType + "</OffsetDelta>");
								System.out.println("</SameFrame>");
							} else if(64 <= frameType && frameType < 128) {
								System.out.println("<SameLocals1StackItemFrame>");
								System.out.println("<OffsetDelta>" + (frameType - 64) + "</OffsetDelta>");
								getVerificationType(ip, 1);
								System.out.println("</SameLocals1StackItemFrame>");
							} else if(frameType == 247) {
								System.out.println("<SameLocals1StackItemFrameExtended>");
								System.out.println("<OffsetDelta>" + ip.readShort() + "</OffsetDelta>");
								getVerificationType(ip, 1);
								System.out.println("</SameLocals1StackItemFrameExtended>");
							} else if(247 < frameType && frameType < 251) {
								System.out.println("<ChopFrame>");
								System.out.println("<OffsetDelta>" + ip.readShort() + "</OffsetDelta>");
								System.out.println("</ChopFrame>");
							} else if(frameType == 251) {
								System.out.println("<SameFrameExtended>");
								System.out.println("<OffsetDelta>" + ip.readShort() + "</OffsetDelta>");
								System.out.println("</SameFrameExtended>");
							} else if(251 < frameType && frameType < 255) {
								System.out.println("<AppendFrame>");
								System.out.println("<OffsetDelta>" + ip.readShort() + "</OffsetDelta>");
								getVerificationType(ip, frameType - 251);
								System.out.println("</AppendFrame>");
							} else if(frameType == 255) {
								System.out.println("<FullFrame>");
								System.out.println("<OffsetDelta>" + ip.readShort() + "</OffsetDelta>");
								numLocal = ip.readShort();
								System.out.println("<NumLocals>" + numLocal + "</NumLocals>");
								getVerificationType(ip, numLocal);
								numStack = ip.readShort();
								System.out.println("<NumStacks>" + numStack + "</NumStacks>");
								getVerificationType(ip, numStack);
								System.out.println("</FullFrame>");
							} else {
								System.err.println("ERROR: Unknown frame_type"); 
								System.exit(0);
							}
						}
						System.out.println("</StackMapTable>");
						...
						System.err.println("ERROR: Unknwon CodeAttribute!!");
						System.out.println("<" + codeAttType + ">");
